...
function initLensTrackerForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const phases = [
            {
                num: "1",
                title: "Fase 1: Evaluación Clínica & Registro Inicial",
                desc: "Atención oftalmológica realizada y orden técnica ingresada al sistema.",
                badge: activePhaseIndex > 0 ? "Completado" : (activePhaseIndex === 0 ? "En Curso" : "Pendiente")
            },
            {
                num: "2",
                title: "Fase 2: Biselado, Tallado Digital & Ensamblaje Óptico",
                desc: "Corte computarizado de cristales, tratamiento antirreflejo y montaje de precisión en montura.",
                badge: activePhaseIndex > 1 ? "Completado" : (activePhaseIndex === 1 ? "En Proceso" : "Pendiente")
            },
            {
                num: "3",
                title: "Fase 3: Control de Calidad & Listo para Entrega",
                desc: `Inspección óptica final y verificación de ejes. Disponible para retiro en ${sedeNombre}.`,
                badge: activePhaseIndex > 2 ? "Completado" : (activePhaseIndex === 2 ? "¡Listo para Retirar!" : "Pendiente")
            },
            {
                num: "4",
                title: "Fase 4: Entregado al Paciente",
                desc: "Lentes entregados satisfactoriamente con estuche rígido y garantía certificada.",
                badge: activePhaseIndex >= 3 ? "¡Entregado con Éxito!" : "Pendiente"
            }
        ];

        const isAllDone = activePhaseIndex >= 3;
        const isReady = activePhaseIndex === 2;
        ...
        resultBox.innerHTML = `
            <div class="tracker-patient-summary">
                ...
                <div>
                    <span class="badge-tag ${(isAllDone || isReady) ? 'badge-green' : 'badge-blue'}" style="font-size: 0.85rem; padding: 0.4rem 0.85rem;">
                        ...
                        ${isAllDone ? 'Lentes Entregados' : (isReady ? '¡Listos para Retiro!' : (activePhaseIndex === 1 ? 'En Fabricación' : 'Evaluación Inicial'))}
                    </span>
                </div>
            </div>
            ...
            <div class="tracker-timeline">
                ${phases.map((ph, idx) => {
                    const isCompleted = idx < activePhaseIndex || (idx === 3 && activePhaseIndex === 3);
